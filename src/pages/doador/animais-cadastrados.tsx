      ...
      >
        ...
        <div className="w-full flex gap-3 items-start">
          {dataAnimal.map(({ id, name, city, details,photos}) => (
            <CardAnimal city={city} details={details} photos={photos} name={name} key={id} id={id}/>
          ))}
        </div>
      </main>
      <Footer />
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const dataAnimal = Array.from(res).map((el) => {
    if (el.publishedAt) {
      const dateFormatted = format(new Date(formatISO(el.publishedAt)), "dd/MM/yyyy HH:mm:ss");
      return {
        ...el,

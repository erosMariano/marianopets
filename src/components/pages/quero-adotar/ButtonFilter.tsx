...
function ButtonFilter({ name, active, activeItemFunc, indexElement }: ButtonFilterProps) {
  ...
  return (
    <button className={`${active && 'font-semibold bg-orange-400 text-white'} w-max text-light-text transition-all p-3 font-medium rounded-lg` } onClick={() => handleSelectType()}>
      {name}
    </button>
  );
}
